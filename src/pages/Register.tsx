...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const data = await api.register({ userName: name, userEmail: email, userPassword: password, userTel: tel });
    ...
    <div className="min-h-screen w-full flex items-center justify-center p-4 py-12">
      <VintageCard className="w-full max-w-lg" delay={0.1}>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <VintageInput type="password" placeholder="Secret Cipher (Password)" value={password} onChange={e => setPassword(e.target.value)} required minLength={6} />
